...
const { hotWallet: getHotFromCrypto } = require('../crypto-client');
...
router.get('/', async (req, res) => {
  ...
  // Ask backend-crypto for its network/contract metadata (single source of truth).
  const cryptoMeta = await getHotFromCrypto().catch(() => null);
  ...
  res.json({
    ...
    platform: {
      network: cryptoMeta?.network || null,
      contract: cryptoMeta?.contract || null,
      fee_bps: env.PLATFORM_FEE_BPS,
    },
  });
});
